...
from backend.games.friends_game.service import log_session_event
...
def test_evaluate_quiz_state_cancels_after_host_timeout(db_session, playing_quiz, sample_host):
    ...
    playing_quiz.host_left_at = utc_now_naive() - HOST_TIMEOUT - timedelta(minutes=1)
    connection_registry.unbind_sid("host-sid-001")
    db_session.commit()

    state = evaluate_quiz_state(db_session, quiz=playing_quiz, now=utc_now_naive())
...
def test_evaluate_quiz_state_expires_resume_before_full_inactivity_cancel(db_session, playing_quiz):
    playing_quiz.last_activity_at = utc_now_naive() - RESUME_WINDOW - timedelta(minutes=1)
    db_session.commit()
...
def test_evaluate_quiz_state_cancels_after_full_inactivity_timeout(db_session, playing_quiz):
    playing_quiz.last_activity_at = utc_now_naive() - INACTIVITY_CANCEL_TIMEOUT - timedelta(minutes=1)
    db_session.commit()
...
def test_mark_participant_left_clears_reconnect_state(sample_player):
    ...
    mark_participant_left(sample_player, left_at=utc_now_naive())
...
def test_log_session_event_is_mirrored_to_file_logs(db_session, sample_quiz, sample_player):
    with patch("backend.games.friends_game.service.log_game_event") as mock_log_game_event:
        log_session_event(
            db_session,
            quiz=sample_quiz,
            ...
            payload={"left_at": utc_now_naive().isoformat()},
        )
